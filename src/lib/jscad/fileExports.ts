...
function stl({ children, partId }: { children: THREE.Object3D[]; partId: string }) {
	...
	const filename = `${partId}.stl`;
	exportGroupToSTL(group, filename);
}
function svg({ geometry, partId }: { geometry: unknown; partId: string }) {
	const svgString = serializeToSvg(geometry);
	const blob = new Blob([svgString], { type: 'image/svg+xml' });
	const link = document.createElement('a');
	link.href = URL.createObjectURL(blob);
	link.download = `${partId}.svg`;
	link.click();
	URL.revokeObjectURL(link.href);
}
